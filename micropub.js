...
const GitHub = {
  ...
  errorResponse(res, body) {
    return {
      error: {
        code: res.statusCode,
        body: { error: res.statusCode, error_description: body.message }
      }
    };
  },
      ...
      GitHub.requestWithOptions(
        ...
        function(err, res, body) {
          ...
          if (res && res.statusCode === 201) {
            resolve({
              code: 201,
              location: `${process.env.SITEURL}/posts/${filename}`
            });
          } else {
      ...
      GitHub.requestWithOptions(
        ...
        function(err, res, body) {
          ...
          if (res && res.statusCode === 201) {
            resolve({
              code: 201,
              location: `${process.env.SITEURL}/assets/${filename}`
            });
          } else {
            resolve(GitHub.errorResponse(res, body));
          }
